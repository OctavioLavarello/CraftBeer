...
const postProduct = async (req: Request, res: Response) => {
  try {
    ...
    if (!degreeOfAlcohol)
      res.status(400).json({ message: "degrees of alcohol is required" });
    if (!description)
      res.status(400).json({ message: "description is required" });
    if (!qualification)
      res.status(400).json({ message: "qualification is required" });
    if (!price) res.status(400).json({ message: "price is required" });
    if (!stock) res.status(400).json({ message: "stock is required" });
    if (!presentation)
      res.status(400).json({ message: "presentation is required" });
